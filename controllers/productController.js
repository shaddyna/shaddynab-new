...
const productController = {
  ...
  getProducts: async (req, res) => {
    try {
      ...
      const products = await Product.find(query).populate('shop', 'name').populate('owner', 'firstName lastName');
      res.status(200).json({
        success: true,
        products
      });
  ...
  getProductsByUser: async (req, res) => {
    try {
      ...
      const products = await Product.find({ owner: userId }).populate('shop', 'name').populate('owner', 'firstName lastName');
      res.status(200).json({
        success: true,
        products
      });
      ...
      res.status(500).json({
        success: false,
        message: 'Error fetching products by user',
        error: error.message
      });
    }
  },
  ...
  getProductsByShop: async (req, res) => {
    try {
      ...
      const products = await Product.find({ 'shop.id': shopId }).populate('shop', 'name').populate('owner', 'firstName lastName');
      res.status(200).json({
        success: true,
        products
      });
  ...
  getProductById: async (req, res) => {
    try {
      const product = await Product.findById(req.params.id).populate('shop', 'name').populate('owner', 'firstName lastName');
      if (!product) {
        return res.status(404).json({ success: false, message: 'Product not found' });
      }
      res.status(200).json({
